...
@api.route('/api/save/action/add/comment', methods = ['POST'])
@login_required
def _d_comment_ActionPlan():
    app.logger.debug('** SWING_CMS ** - API Appointment Detail')
    try:
        # POST: Save Appointment
        if request.method == 'POST':
            ...
            actionplan = ActionPlan.query.filter_by(id = actionPlan).first()
            actionplan.comment_by = current_user.id
...
def _d_save_info_company():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            phone = request.json['phone']
            instagram = request.json['txt_instagram']
            facebook = request.json['txt_facebook']
            web  = request.json['txt_web']
...
@api.route('/api/diagnostico/', methods = ['POST'])
# @login_required
def _d_diagnosticorespuestas():
    app.logger.debug('** SWING_CMS ** - API Appointment Detail')
    try:
        # POST: Save Appointment
        if request.method == 'POST':
            ...
            db.session.commit()
            print(resultados)
            return jsonify({ 'status': 200, 'msg': 'Perfil actulizado con','id':diagnosis.id })
    except Exception as e:
        app.logger.error('** SWING_CMS ** - API Appointment Detail Error: {}'.format(e))
        return jsonify({ 'status': 'error', 'msg': e })
...
def get_companies_sde_1():
    try:
        ...
        companies_surveys = (
            ...
            .filter(surveys_sde.catalog_surveys_id == 1,surveys_sde.created_by==3)
            .all()
        )
