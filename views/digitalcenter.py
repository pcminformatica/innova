...
@digitalcenter.route('/encuesta/im/<int:company_id>/encuestas',methods=['GET', 'POST'])
def _encuesta_verificacion(company_id):
...
@digitalcenter.route('/empresas/etapa3',methods=['GET', 'POST'])
@login_required
@nocache
def _company_monitoring_list_etapa3():
    app.logger.debug('** SWING_CMS ** - ------------------')
    #diagnosis = DiagnosisCompany.query.filter_by(created_by=current_user.id)
    lista = []
    data = []
