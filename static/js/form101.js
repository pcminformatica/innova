...
submitButton.addEventListener('click', function() {
    ...
    swcms.mdcCheckbox.forEach((sel) => {
      ...
      var claves = ['txt_name',
        ...
        'txt_company_name',]
      var respuestas = [];
        var texts,selects = false
      claves.forEach((property_name) => {
        if (mdcAssignedVars[property_name].value === '') {   
            showMSJ('Por favor responda la pregunta:', mdcAssignedVars[property_name].label.root.attributes.hiddenlabel.value, 'info');
            return false;
        } else {
            preguntas.push({
                "id": property_name,
                "pregunta": mdcAssignedVars[property_name].label.root.attributes.hiddenlabel.value,
                "respuesta": mdcAssignedVars[property_name].value,
                });
            }
            texts = true
        })
        var claves = ['txt_departamento',
        'txt_municipio',
        'txt_aldea',
        ]
        claves.forEach((property_name) => {
            if (mdcAssignedVars[property_name].value === '') {   
                ...
                preguntas.push({
                    "id": property_name,
                    "pregunta": mdcAssignedVars[property_name].label.root.attributes.hiddenlabel.value,
                    "respuesta": mdcAssignedVars[property_name].selectedText.textContent,
                });
            }
            selects = true
        })
