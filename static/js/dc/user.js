...
function initializeRTC () {
    ...
    socket.on('RTCUserList2', (data) => {
        console.log('RTC User List RTCUserList2');
        console.log(data);
...
class rtcPeerConnection {
    ...
    constructor(init, uListElem) {
        ...
        this.rtcSimplePeer.on('signal', (data) => {
            console.log(data);
            console.log('SimplePeer User Connection Class 1');
            if (this.isInitiator) {
...
function establishRTC(init = true, receiverData = null) {
    ...
    if (init) {
        uListElem = this;
        console.log('Establish Initiator Peer4');
    } else {
        console.log('Establish Initiator Peer2');
    ...
    if (init) {
        ...
        console.log('Establish Initiator 112');
...
function createRTCListUserContainer(parentContainer) {
    ...
    switch (parentContainer) {
        ...
        case 'itc':
            userContainer.addEventListener('click', establishRTC);
            document.querySelector('#itc-active-rooms').appendChild(userContainer);
            break;
...
function showRTCUserList(userlist) {
    ...
    userlist.rtc_online_users.itc_users.forEach((user) => {
        console.log('123');
        if (user.id != swcms.advStreams.myUserInfo.id) {
            appendRTCUser(user.r_id, user, 'itc');
            appendRTCTransferList(user);
            rtcULID.push('l_' + user.id);
            rtcUTID.push('tl_' + user.id);
        } else {
